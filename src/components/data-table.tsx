...
import { Activity, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Download, Loader2, Search } from 'lucide-react';
...
interface DataTableProps<T extends { id: string }> {
  ...
  page?: number;
  totalPages?: number;
  /** Items requested per page (e.g. 10). Shown in the pagination footer. */
  pageSize?: number;
...
export function DataTable<T extends { id: string }>({
  ...
  // Keep typing responsive; only notify parent after debounce so lists don't refetch every keystroke.
  ...
  // Full skeleton only on first load (no rows yet). Refetch/search keeps the table mounted.
  ...
  return (
    <div className="space-y-4">
      ...
      {showInitialSkeleton ? (
        ...
        <>
          <div
            className={`overflow-hidden rounded-card border border-brand-border/80 bg-surface shadow-soft transition-opacity ${
              showRefreshing ? 'opacity-70' : 'opacity-100'
            }`}
          >
                ...
                <thead className="sticky top-0 z-10 bg-surface-elevated/95 backdrop-blur">
                  <tr>
                    ...
                    {columns.map((col) => (
                      <th
                        key={col.key}
                        className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.14em] text-text-muted"
                      >
                        {col.header}
                      </th>
                    ))}
